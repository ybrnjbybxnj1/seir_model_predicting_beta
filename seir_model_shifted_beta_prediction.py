...
def load_and_prepare_data(data_dir, num_seeds, test_seed=None):
    ...
    for i in range(num_seeds):
        ...
        df['prev_I'] = df['I'].shift(-2).fillna(0)  # shift I for 2 days back
        df['seed'] = i
...
def predict_beta_with_shift(model, day, prev_I, current_beta, shift_factor=0.1, min_beta=1e-7):
    log_beta = model.predict([[day, prev_I]])
    predicted_beta = np.exp(log_beta)[0]

    shifted_beta = predicted_beta + shift_factor * (current_beta - predicted_beta)
    
    return max(shifted_beta, min_beta)
...
def simulate_seir(seed_data, start_day, model, max_days=200, stop_early=True):
    ...
    beta = predict_beta_with_shift(model, start_day, prev_I, Beta)
    ...
    for day in range(1, max_days + 1):
        ...
        beta = predict_beta_with_shift(model, current_day, prev_I_sim, beta)
