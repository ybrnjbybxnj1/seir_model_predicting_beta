...
def train_and_save_model(data_dir, num_seeds, model_save_path, exclude_seed=None, degree=2):
    ...
    # Optionally, write the training time to a CSV file.
...
def predict_multiple_seeds(seeds, start_day, data_dir, model_path, results_csv='prediction_results.csv'):
    '''Predict and plot for multiple seeds in subplots, with time measurement and saving results'''
...
if __name__ == "__main__":
    ...
    DATA_DIR = '../data/' # ADD DIRECTORY HERE
    NUM_SEEDS = 1500
    MODEL_PATH = 'NAME.joblib' # CHANGE THE NAME OF THE MODEL
    PREDICT_DATA_DIR = '../data/' # ADD DIRECTORY HERE
    
    # --- Training Section ---
    # Uncomment the next lines to train the model.
    # Training time is measured and saved to "training_results.csv"
    model, training_time = train_and_save_model(DATA_DIR, NUM_SEEDS, MODEL_PATH, degree=3)
    
    # --- Prediction Section ---
    # Predict on the first 10 seeds.
    # This will also measure the prediction time per seed and overall,
    # then write the results to "prediction_results.csv"
