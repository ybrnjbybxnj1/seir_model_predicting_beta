...
def load_and_prepare_data(data_dir, num_seeds, test_seed=None):
    '''A FUNCTION TO LOAD AND PREPARE THE DATA'''
...
def load_saved_model(model_path):
    '''A FUNCTION TO LOAD SAVED MODEL'''
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"Model file not found at {model_path}")
    return joblib.load(model_path)

def predict_beta(model, day, min_beta=1e-7):
    '''A FUNCTION TO PREDICT BETA VALUES'''
...
def simulate_seir(seed_data, start_day, model, max_days=200):
    '''A FUNCTION TO CALCULATE SEIR TRAJECTORIES WITH BOUNDS(MAX_DAYS) FOR IT'''
...
def plot_results(seed_data, history, start_day, seed):
    '''A FUNCTION TO PLOT PREDICTION RESULTS BOTH WITH ACTUAL DATA'''
...
if __name__ == "__main__":
    DATA_DIR = '../data/' # PATH FOR THE DATA DIRECTORY
    NUM_SEEDS = 1500
    MODEL_PATH = 'name.joblib' # PATH TO SAVE THE MODEL
    
    '''UNCOMMENT IF YOU NEED TO TRAIN AND SAVE THE MODEL'''
    ...
    DATA_DIR_TO_PREDICT = '../data/' # PATH TO THE DIRECTORY WITH DATA TO PREDICT ON
    
    predict_on_seed(seed=1, start_day=50, data_dir=DATA_DIR_TO_PREDICT, model_path=MODEL_PATH)
